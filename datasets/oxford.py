...
class OxfordDataset(Dataset):
    ...
    def __init__(self, config, split='train'):
        ...
        for seq in self.sequences:
            ...
            self.seq_idx_range[seq] = [len(self.frames), len(self.frames) + len(seq_frames)]    # {'nuScenes_121': [0, 38], 'nuScenes_122': [38, 76]}
            self.seq_lens.append(len(seq_frames))   # [38, 38]
            self.frames.extend(seq_frames)  # len(self.frames) = 76
...
class nuScenesDataset(OxfordDataset):
    def __init__(self, config, split='train'):
        ...
        for seq in self.sequences:
            ...
            self.seq_idx_range[seq] = [len(self.frames), len(self.frames) + len(seq_frames)]    # {'nuScenes_123': [0, 38]}
            self.seq_lens.append(len(seq_frames))   # [38]
            self.frames.extend(seq_frames)  # ['1526915283430217.jpg',..]
